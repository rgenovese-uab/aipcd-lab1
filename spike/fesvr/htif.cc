...
htif_t::htif_t()
  : mem(this), entry(DRAM_BASE), sig_addr(0), sig_len(0),
    tohost_addr(0), fromhost_addr(0), clearmip_addr(0), exitcode(0), stopped(false),
    syscall_proxy(this)
{
...
void htif_t::load_program()
{
  ...
  if (symbols.count("clearmip")){
    clearmip_addr = symbols["clearmip"];
  }else {
    fprintf(stderr, "warning: clearmip symbol not in ELF\n"); 
  }
...
void htif_t::set_tohost_addr(addr_t tohost_addr, addr_t fromhost_addr)
{
    this->tohost_addr = (addr_t) tohost_addr;
    this->fromhost_addr = (addr_t) fromhost_addr;
}

void htif_t::set_clearmip_addr(addr_t clearmip_addr)
{
    this->clearmip_addr = (addr_t) clearmip_addr;
}
