...
#define DISASM_RINSN_AND_ROUND(code) \
  ...
  if (isa->extension_enabled(EXT_XGMX)) {
    ...
    DISASM_INSN("gmx.tbq", gmx_tbq, 0, {&xrd});
    DISASM_INSN("gmx.psr", gmx_psr, 0, {&xrd, &xrs1});
    DISASM_INSN("gmx.psrmin", gmx_psrmin, 0, {&xrd, &xrs1});
    DISASM_INSN("gmx.bpm", gmx_bpm, 0, {&xrd, &xrs1, &xrs2});
    DISASM_INSN("gmx.pig", gmx_pig, 0, {&xrd, &xrs1, &xrs2});
  }
