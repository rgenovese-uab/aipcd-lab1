...
for ( int i = 0; i < 32; i++ ) {
  ...
  uint64_t ph_out = ( ph >> 31 ) & 1;
  uint64_t mh_out = ( mh >> 31 ) & 1;

  ph_out_v = ph_out_v | ( ph_out << i );
  mh_out_v = mh_out_v | ( mh_out << i );
...
for ( int i = 0; i < 32; i++ ) {
  delta_h = delta_h << 1;
  delta_h = delta_h | ( ( mh_out_v  >> ( 31 - i ) ) & 1ul );
  delta_h = delta_h << 1;
  delta_h = delta_h | ( ( ph_out_v >> ( 31 - i ) ) & 1ul );
}

WRITE_RD(sext_xlen(delta_h));