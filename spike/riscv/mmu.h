...
# define READ_MEM(addr, size) \
  ...
  template<typename T>
  T ALWAYS_INLINE load(reg_t addr, uint32_t xlate_flags = 0, int tval_type = 0) {
    target_endian<T> res;
    reg_t vpn = addr >> PGSHIFT;
    bool aligned = (addr & (sizeof(T) - 1)) == 0;
    bool tlb_hit = tlb_load_tag[vpn % TLB_ENTRIES] == vpn && !DISABLE_PGSIZE_CHECK;

    if (likely(xlate_flags == 0 && aligned && tlb_hit)) {
      res = *(target_endian<T>*)(tlb_data[vpn % TLB_ENTRIES].host_offset + addr);
    } else {
      load_slow_path(addr, sizeof(T), (uint8_t*)&res, xlate_flags, tval_type);
    }

    if (proc) {
      proc->state.vaddr = addr;
...
# define WRITE_MEM(addr, val, size) \
  ...
  template<typename T>
  void ALWAYS_INLINE store(reg_t addr, T val, uint32_t xlate_flags = 0, int tval_type = 0) {
    reg_t vpn = addr >> PGSHIFT;
    bool aligned = (addr & (sizeof(T) - 1)) == 0;
    bool tlb_hit = tlb_store_tag[vpn % TLB_ENTRIES] == vpn && !DISABLE_PGSIZE_CHECK;

    if (xlate_flags == 0 && likely(aligned && tlb_hit)) {
      *(target_endian<T>*)(tlb_data[vpn % TLB_ENTRIES].host_offset + addr) = to_target(val);
    } else {
      target_endian<T> target_val = to_target(val);
      store_slow_path(addr, sizeof(T), (const uint8_t*)&target_val, xlate_flags, true, false, tval_type);
    }

    if (proc) {
      proc->state.vaddr      = addr;
      proc->state.store_data = (reg_t) val;
      // full 1s mask
      reg_t full_mask = ~((reg_t) 0);
      // Avoid warning checking val size
      // Return full mask in case the val size is equal to reg_t size
      reg_t val_mask = (sizeof(val) < sizeof(reg_t)) ? ~(full_mask << (reg_t)(sizeof(val)*8)) : full_mask;
      proc->state.store_mask = val_mask;
  ...
  template<typename T>
  void ALWAYS_INLINE sneaky_store_(reg_t addr, T val, uint32_t xlate_flags = 0) {
    reg_t vpn = addr >> PGSHIFT;
    bool aligned = (addr & (sizeof(T) - 1)) == 0;
    bool tlb_hit = tlb_store_tag[vpn % TLB_ENTRIES] == vpn && !DISABLE_PGSIZE_CHECK;
    ...
    if (xlate_flags == 0 && likely(aligned && tlb_hit)) {
      *(target_endian<T>*)(tlb_data[vpn % TLB_ENTRIES].host_offset + addr) = to_target(val);
      ...
      target_endian<T> target_val = to_target(val);
      sneaky_store(addr, sizeof(T), (const uint8_t*)&target_val); //[TOCHECK] from KA
    }
  ...
  void sneaky_store(reg_t addr, reg_t len, const uint8_t* val);
  reg_t sneaky_translate(reg_t addr, reg_t len, access_type type);
  reg_t tlb_translate(reg_t addr, reg_t len, access_type type, reg_t satp, reg_t priv_lvl, reg_t mstatus, reg_t* exc_error);
  reg_t sneaky_walk(reg_t addr, access_type type, reg_t mode);
  ...
  bool store_conditional(reg_t addr, T val)
  {
    ...
    //This line has been added here in case SC doesn't have a valid load reservation.
    //TODO: Improve?
    proc->state.vaddr = addr;
...
private:
  ...
  reg_t s2xlate(reg_t gva, reg_t gpa, access_type type, access_type trap_type, bool virt, bool hlvx, int tval_type = 0);

  // perform a page table walk for a given VA; set referenced/dirty bits
  reg_t walk(reg_t addr, access_type type, reg_t prv, bool virt, bool hlvx, int tval_type = 0);
  reg_t tlb_walk(reg_t addr, access_type type, reg_t satp, reg_t mode, reg_t mstatus, reg_t* exc_error);

  // handle uncommon cases: TLB misses, page faults, MMIO
  tlb_entry_t fetch_slow_path(reg_t addr);
  void load_slow_path(reg_t addr, reg_t len, uint8_t* bytes, uint32_t xlate_flags, int tval_type = 0);
  void load_slow_path_intrapage(reg_t addr, reg_t len, uint8_t* bytes, uint32_t xlate_flags, int tval_type = 0);
  void store_slow_path(reg_t addr, reg_t len, const uint8_t* bytes, uint32_t xlate_flags, bool actually_store, bool require_alignment, int tval_type = 0);
  void store_slow_path_intrapage(reg_t addr, reg_t len, const uint8_t* bytes, uint32_t xlate_flags, bool actually_store, int tval_type = 0);
  ...
  reg_t translate(reg_t addr, reg_t len, access_type type, uint32_t xlate_flags, int tval_type = 0);

  // ITLB lookup
  inline tlb_entry_t translate_insn_addr(reg_t addr) {
    reg_t vpn = addr >> PGSHIFT;
    if (likely(tlb_insn_tag[vpn % TLB_ENTRIES] == vpn) && !DISABLE_PGSIZE_CHECK)
