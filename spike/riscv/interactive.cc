...
void sim_t::interactive()
{
  ...
  funcs["set_mip"] = &sim_t::interactive_mip;
  funcs["set_freg"] = &sim_t::interactive_set_freg;
  funcs["h"] = funcs["help"];
...
void sim_t::interactive_mip(const std::string& cmd, const std::vector<std::string>& args)
{
  if (args.size() < 1)
    throw trap_interactive();
  ...
  reg_t mip_value = strtol(arg1.c_str(),NULL,16);

  std::ostream out(sout_.rdbuf());
  out << std::hex;

  p->set_mip_ei(mip_value);

  out <<" SET MIP "<< p->get_state()->mip->read()<< std::endl;
}

void sim_t::interactive_set_freg(const std::string& cmd, const std::vector<std::string>& args)
{
  if (args.size() < 1)
    throw trap_interactive();
  ...
  reg_t freg_dst = strtol(arg1.c_str(),NULL,16);
  reg_t freg_value = strtol(arg2.c_str(),NULL,16);

  std::ostream out(sout_.rdbuf());
  out << std::hex;

  float128_t v;
  v.v[1] = p->get_state()->FPR[freg_dst].v[1];
  v.v[0] = freg_value;

  p->get_state()->FPR.write(freg_dst,v);

  freg_t r = p->get_state()->FPR[freg_dst];

  //fprintf(stdout, "UPDATING FP REG[%d] with value %lx - RESULT %lx \n", reg_dst, val, p_state->FPR[reg_dst]);
  out <<" SETTING FREG[" << arg1 << "]=" << " 0x" << std::setfill ('0') << std::setw(16) << r.v[1] << std::setw(16) << r.v[0] << std::endl;
}
