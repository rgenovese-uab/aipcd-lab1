    ...
    rom_created(false)
{
  ...
  for (size_t i = 0; i < cfg->nprocs(); i++) {
    procs[i] = new processor_t(&isa, cfg->varch(), cfg->core_type(), this, cfg->hartids()[i], halted, cfg->reset_vector(), // TODO: array of core_types for heterogeneous system
                               cfg->endianness, cfg->has_mboot_main_id, cfg->mboot_main_id_val(), log_file.get(), sout_);
  }
  ...
  if (fdt_parse_clint(fdt, &clint_base, "riscv,clint0") == 0) {
    ...
    fprintf(stdout, "SPIKE SIM.CC - ADDED CLINT AT %lx \n", clint_base);
  }
  ...
  if (fdt_parse_plic(fdt, &plic_base, &plic_ndev, "riscv,plic0") == 0) {
    ...
    intctrl = plic.get();
    fprintf(stdout, "SPIKE SIM.CC - ADDED PLIC AT %lx NDEV %lx \n", plic_base, plic_ndev );
  }
  ...
  if (fdt_parse_ns16550(fdt, &ns16550_base,
                        &ns16550_shift, &ns16550_io_width, "ns16550") == 0) {
    assert(intctrl);
    ...
    fprintf(stdout, "SPIKE SIM.CC - ADDED UART AT %lx \n", ns16550_base );
  }
...
// #else
//     steps = 1; //std::min(n - i, INTERLEAVE - current_step);
// #endif
    steps = std::min(n - i, INTERLEAVE - current_step);
    ...
    {
      current_step = 0;
      //procs[current_proc]->get_mmu()->yield_load_reservation(); //[TOCHECK] from lagarto KA, causing issues in ka uvm
...
{
  reg_t aux;
  aux = (reg_t) ((long int) addr >> MAX_PADDR_BITS);
  return (aux == 0 || aux == -1);
...
void sim_t::set_rom()
{
    if (rom_created) return;
    int reset_vec_size = 7;
    uint32_t *reset_vec;
    uint32_t default_reset_vec[reset_vec_size] = {
            0x0010041b,            //addiw s0,zero,129  //these first two correspond to li s0, DRAM_BASE
            0x01f41413,            //slli  s0,s0,0x1f
            0xf1402573,            //csrr  a0,mhartid
            0x00000597,            //li  a1,384         //check if this is the dts addr (la a1, _dtb)
            0x07458593,            // addi  a1,a1,116 # 1080 <_dtb>
            0x00040067,            //jr  s0             //jump to DRAM_BASE
            0
    };
    uint32_t * reset_vec_file = NULL;
    if (cfg->bootrom_file()) {
        FILE* fd = fopen(cfg->bootrom_file(), "r");
        if (fd == NULL) {
            fprintf(stderr, "Error opening bootrom file (%s)\n", cfg->bootrom_file());
            exit(-1);
        }
        struct stat s;
        if (stat(cfg->bootrom_file(), &s) < 0)
            abort();
        size_t size = s.st_size;
        reset_vec_size = size / sizeof(uint32_t); // Number of 32bit words
        uint32_t * reset_vec_file = (uint32_t *) malloc(size);
        if (!reset_vec_file) {
            fprintf(stderr, "Cannot allocate memory\n");
            exit(-1);
        }
        int res = fread(reset_vec_file, 4, reset_vec_size, fd);
        if (res != reset_vec_size) {
            fprintf(stderr, "Error reading from bootrom file (%s), expected (%d) bytes, read (%d)\n", cfg->bootrom_file(), size, res);
            exit(-1);
        }
        reset_vec = reset_vec_file;
        fprintf(stderr, "Read bootrom from `%s`\n", cfg->bootrom_file());
    } else {
        reset_vec = default_reset_vec;
    }
  ...
  std::vector<char> rom((char*)reset_vec, (char*)reset_vec + reset_vec_size * sizeof(uint32_t));
  ...
  fprintf(stderr, "Adding bootrom to pc: %llx\n", cfg->reset_vector());
  bus.add_device(cfg->reset_vector(), boot_rom.get());
  if (reset_vec_file)
      free(reset_vec_file);
