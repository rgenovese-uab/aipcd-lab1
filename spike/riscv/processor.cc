...
processor_t::processor_t(const isa_parser_t *isa, const char* varch, core_type_t core,
                         simif_t* sim, uint32_t id, bool halt_on_reset,
                         reg_t reset_vector,
                         memif_endianness_t endianness,
                         bool has_mboot_main_id, reg_t mboot_main_id_val,
                         FILE* log_file, std::ostream& sout_)
  : debug(false), halt_request(HR_NONE), isa(isa), sim(sim), id(id), xlen(0),
  histogram_enabled(false), log_commits_enabled(false),
  log_file(log_file), sout_(sout_.rdbuf()), halt_on_reset(halt_on_reset),
  reset_vector(reset_vector),
  has_mboot_main_id(has_mboot_main_id), mboot_main_id_val(mboot_main_id_val),
...
{
  pc = proc->reset_vector;
  XPR.reset();
  FPR.reset();

  proc->get_state()->last_fflags = 0;
  ...
  csrmap[CSR_MTVEC] = mtvec = std::make_shared<tvec_csr_t>(proc, CSR_MTVEC, proc->core_type == STANDARD ? 0 : (proc->core_type == LAGARTO_OX ? proc->reset_vector : proc->reset_vector + 0x40));
  csrmap[CSR_MCAUSE] = mcause = std::make_shared<cause_csr_t>(proc, CSR_MCAUSE);
  csrmap[CSR_GMX_P] = gmx_p = std::make_shared<basic_csr_t>(proc, CSR_GMX_P, 0);
  csrmap[CSR_GMX_T] = gmx_t = std::make_shared<basic_csr_t>(proc, CSR_GMX_T, 0);
  csrmap[CSR_GMX_POS] = gmx_pos = std::make_shared<basic_csr_t>(proc, CSR_GMX_POS, 0);
  csrmap[CSR_GMX_TB_LO]  = gmx_tb_lo  = std::make_shared<basic_csr_t>(proc, CSR_GMX_TB_LO, 0);
  csrmap[CSR_GMX_TB_HI]  = gmx_tb_hi  = std::make_shared<basic_csr_t>(proc, CSR_GMX_TB_HI, 0);
  ...
  if (proc->has_mboot_main_id) {
     csrmap[CSR_MBOOT_MAIN_ID] = std::make_shared<const_csr_t>(proc, CSR_MBOOT_MAIN_ID, proc->mboot_main_id_val);
  }
...
void processor_t::vectorUnit_t::reset()
{
  ...
  if (p->is_vpu()) {
      memset(reg_file, 0xff, NVPR * vlenb);
  } else {
      memset(reg_file, 0, NVPR * vlenb);
  }
  mem_addr_array = (uint64_t *) calloc((VLEN/8), sizeof(uint64_t));
  if (mem_addr_array == NULL) {
    std::cout << "Not enough memory, malloc failed (mem_addr_array)" << std::endl;
    throw 12; // ENOMEM
  }
  mem_data_array = (uint64_t *) calloc((VLEN/8), sizeof(uint64_t)); // TODO optimize
  if (mem_data_array == NULL) {
    std::cout << "Not enough memory, malloc failed (mem_data_array)" << std::endl;
    throw 12; // ENOMEM
  }
  ...
  //[TOCHECK] from KA missing altfp, altfp_fcvt
...
reg_t processor_t::vectorUnit_t::set_vl(int rd, int rs1, reg_t reqVL, reg_t newType)
{
  ...
  if (vtype->read() != newType) {
    ...
    vill = !(vflmul >= 0.125 && vflmul <= 8)
           ...
           || (newType >> 8) != 0
           || (p->core_type != STANDARD && vta != 1)
           || (p->core_type != STANDARD && vflmul > 1);
  ...
  if (vlmax == 0) {
    ...
    vl->write_raw(vlmax);
  } else if (rs1 != 0) {        //[TOCHECK] conditions from KA
    if( reqVL <= vlmax )
      vl->write_raw( reqVL);
    else if( reqVL < (2*vlmax))
      vl->write_raw(ceil(((double)reqVL)/2));
    else if( reqVL >= (2*vlmax))
      vl->write_raw( vlmax );
...
void processor_t::reset()
{
  ...
  halt_on_reset = false;
  //set_csr(CSR_MSTATUS, state.mstatus); //[TOCHECK] from KA
  VU.reset();
...
void processor_t::set_mip_ei(reg_t val) //[TOCHECK] from KA
{
  val = zext_xlen(val);
  reg_t external_interrupts = extension_enabled('S') ? MIP_SEIP | MIP_MEIP | MIP_MSIP: MIP_MEIP | MIP_MSIP;
  reg_t mask = external_interrupts;
  //fprintf(stdout, "PROCESSOR.CC SETTING MIP - VAL %x MASK %x EXTERNAL INTERRUPTS %x PREV MIP %x\n", val, mask, external_interrupts, state.mip->read() );
  
  /*
  reg_t val_ored = ( mip_seip_sw & MIP_SEIP ) | val;
  mip_seip_ext = val & MIP_SEIP;
  state.mip = (state.mip & ~mask) | (val_ored & mask);
  */
  state.mip->backdoor_write_with_mask(mask, val);// = (state.mip->read() & ~mask) | (val_ored & mask);
  //fprintf(stdout, "PROCESSOR.CC MIP = %x PRV LEVEL %d \n", state.mip->read(), state.prv);
  //std::cout<<"MIP = "<<std::hex<<state.mip<<"PRV LEVEL "<<state.prv <<std::endl;
}

reg_t processor_t::get_prv_lvl(){
  return state.prv;
}

void processor_t::put_csr(int which, reg_t val)   //[TOCHECK] in KA unimplemented from CSR_PMPADDR0, CSR_PMPCFG0, CSR_MIP,
                                                  //CSR_SCOUNTEREN, CSR_MCOUNTEREN, CSR_TSELECT, CSR_TDATA1, CSR_TDATA2, CSR_DCSR, CSR_DPC,
                                                  //CSR_DSCRATCH, CSR_DSCRATCH + 1, CSR_TO_HOST, CSR_FROM_HOST, CSR_CLEARMIP 
{
  val = zext_xlen(val);

  if (which >= CSR_PMPADDR0 && which < CSR_PMPADDR0 + state.max_pmp) {
    // Not implemented in Ka
 ...
 else if (which >= CSR_PMPCFG0 && which < CSR_PMPCFG0 + state.max_pmp / 4) {
    // Not implemented in Ka
  ...
  else if ( which == CSR_MIP ){
    reg_t val_ored = (mip_seip_ext & MIP_SEIP) | val;
    mip_seip_sw = val & MIP_SEIP;
  ...
  else if(  which == CSR_SCOUNTEREN | which == CSR_MCOUNTEREN | which == CSR_TSELECT | 
            which == CSR_TDATA1 | which == CSR_TDATA2 | which == CSR_TDATA3 | which == CSR_DPC | 
            which == CSR_DSCRATCH0 | which == CSR_DSCRATCH1 | which == CSR_DCSR){
    return;
  }
  else if( which == CSR_TO_HOST ){
    sim->csr_tohost(val);
    return;
  } 
  else if( which == CSR_FROM_HOST ){
    return;
  } 
  else if( which == CSR_CLEARMIP ){
    //std::cout<<"CLEARING MIP FROM SPIKE"<<std::endl;
    set_mip_ei(0);
    return;
  } 
  else if( which == CSR_MSTATUS ){
    if ((val ^ state.mstatus->read()) &
          (MSTATUS_MPP | MSTATUS_MPRV | MSTATUS_SUM | MSTATUS_MXR))
        mmu->flush_tlb();

      bool has_fs = extension_enabled('S') || extension_enabled('F')
                  || extension_enabled('V');
      bool has_vs = extension_enabled('V');

      reg_t mask = MSTATUS_SIE | MSTATUS_SPIE | MSTATUS_MIE | MSTATUS_MPIE
                 | MSTATUS_MPRV
                 | (extension_enabled('S') ? MSTATUS_SUM : 0)
                 | MSTATUS_MXR | MSTATUS_TW | MSTATUS_TVM | MSTATUS_TSR
                 | (has_fs ? MSTATUS_FS : 0)
                 | (has_vs ? MSTATUS_VS : 0)
                 | (custom_extensions.size() ? MSTATUS_XS : 0); //[TOCHECK] from lagarto KA ext -> custom_extensions.size()

      reg_t requested_mpp = legalize_privilege(get_field(val, MSTATUS_MPP));
      state.mstatus->write(set_field(state.mstatus->read(), MSTATUS_MPP, requested_mpp));
      if (extension_enabled('S'))
        mask |= MSTATUS_SPP;

      state.mstatus->write( (state.mstatus->read() & ~mask) | (val & mask) );

      bool dirty = (state.mstatus->read() & MSTATUS_FS) == MSTATUS_FS;
      dirty |= (state.mstatus->read() & MSTATUS_XS) == MSTATUS_XS;
      dirty |= (state.mstatus->read() & MSTATUS_VS) == MSTATUS_VS;
      if (isa->get_max_xlen() == 32)
        state.mstatus->write( set_field(state.mstatus->read(), MSTATUS32_SD, dirty));
      else
        state.mstatus->write( set_field(state.mstatus->read(), MSTATUS64_SD, dirty));

      if (extension_enabled('U'))
        state.mstatus->write( set_field(state.mstatus->read(), MSTATUS_UXL, xlen_to_uxl(isa->get_max_xlen())));
      if (extension_enabled('S'))
        state.mstatus->write( set_field(state.mstatus->read(), MSTATUS_SXL, xlen_to_uxl(isa->get_max_xlen())));
...
reg_t processor_t::get_csr(int which, insn_t insn, bool write, bool peek)
{
  if (which >= CSR_PMPADDR0 && which < CSR_PMPADDR0 + state.max_pmp) {
    ...
    // Not implemented in Ka //[TOCHECK] meld, present in EPI
    return 0;
  }
  else if(  which == CSR_SCOUNTEREN | which == CSR_MCOUNTEREN | which == CSR_TSELECT | 
            which == CSR_TDATA1 | which == CSR_TDATA2 | which == CSR_TDATA3 | which == CSR_DPC | 
            which == CSR_DSCRATCH0 | which == CSR_DSCRATCH1 | which == CSR_MARCHID | which == CSR_DCSR){
    return 0 ; //not implemented in KA
  }
  else if( which == CSR_TO_HOST ){
    return state.to_host;
  } 
  else if( which == CSR_FROM_HOST ){
    return state.from_host;
  } 
  else if( which == CSR_CLEARMIP ){
    return 0;
  } 
...
reg_t illegal_instruction(processor_t UNUSED *p, insn_t insn, reg_t UNUSED pc)
{
  ...
  p->trap_illegal = 1;
  throw trap_illegal_instruction(0 & 0xffffffffULL);
}
