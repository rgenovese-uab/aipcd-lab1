...
#define VI_TAIL_AGNOSTIC_REDUCTION(eew) \
 VI_TAIL_AGNOSTIC_COMMON(eew, 1, P.VU.vstart->read(), 0, 1)

#define VI_TAIL_AGNOSTIC_LD(elt_width, is_mask_ldst) \
 if (is_mask_ldst) {VI_TAIL_AGNOSTIC_COMMON(1, P.VU.vl->read(), P.VU.vstart->read(), is_mask_ldst, 0)} \
 else if (sizeof(elt_width##_t) == 1) {VI_TAIL_AGNOSTIC_COMMON(e8, P.VU.vl->read(), P.VU.vstart->read(), is_mask_ldst, 0)} \
 else if (sizeof(elt_width##_t) == 2) {VI_TAIL_AGNOSTIC_COMMON(e16, P.VU.vl->read(), P.VU.vstart->read(), is_mask_ldst, 0)} \
 else if (sizeof(elt_width##_t) == 4) {VI_TAIL_AGNOSTIC_COMMON(e32, P.VU.vl->read(), P.VU.vstart->read(), is_mask_ldst, 0)} \
 else if (sizeof(elt_width##_t) == 8) {VI_TAIL_AGNOSTIC_COMMON(e64, P.VU.vl->read(), P.VU.vstart->read(), is_mask_ldst, 0)} \
 else {printf("LD Tail: Unsupported size!\n");}

#define VI_TAIL_AGNOSTIC_MASK \
 VI_TAIL_AGNOSTIC_COMMON(1, P.VU.vl->read(), P.VU.vstart->read(), 1, 0)

//don't multiple by 2, shift by one bit.
#define VI_TAIL_AGNOSTIC_BASIC(is_widening) \
      if (is_widening == 0) { \
         VI_TAIL_AGNOSTIC_COMMON(P.VU.vsew, P.VU.vl->read(), P.VU.vstart->read(), 0, 0) \
      } else { \
         VI_TAIL_AGNOSTIC_COMMON((P.VU.vsew<<1), P.VU.vl->read(), P.VU.vstart->read(), 0, 0) \
      }

#define VI_TAIL_AGNOSTIC_COMMON(eew, vl, vstart, is_mask, is_reduction) \
  if ((P.VU.vta || is_mask) && (vstart < vl)) {\
    float vemul = is_mask ? 1 : (((float) eew / P.VU.vsew) * P.VU.vflmul); \
    reg_t emul = vemul < 1 ? 1 : vemul; \
    if (is_reduction) emul = 1; \
    if (P.core_type != STANDARD) emul = 1; /* Tail undisturbed in second register of widening instructions */ \
    for (reg_t i = vl; i < P.VU.VLEN/eew * emul; ++i) { \
      if (eew == e8) { \
          uint8_t &vd = P.VU.elt<uint8_t>(insn.rd(), i, true); \
          vd = 0xff; \
      } else if (eew == e16) { \
          uint16_t &vd = P.VU.elt<uint16_t>(insn.rd(), i, true); \
          vd = 0xffff; \
      } else if (eew == e32) { \
        uint32_t &vd = P.VU.elt<uint32_t>(insn.rd(), i, true); \
          vd = 0xffffffff; \
      } else if (eew == e64) { \
          uint64_t &vd = P.VU.elt<uint64_t>(insn.rd(), i, true); \
          vd = 0xffffffffffffffff; \
      } else if (eew == 1) { \
          uint8_t &vd = P.VU.elt<uint8_t>(insn.rd(), i / 8, true); \
          vd |= (1 << (i % 8)) ; \
      } \
    } \
  }
...
#define VI_LOOP_END \
  ...
  VI_TAIL_AGNOSTIC_BASIC(0)  \
  P.VU.vstart->write(0);

#define VI_WIDENING_LOOP_END \
  } \
  VI_TAIL_AGNOSTIC_BASIC(1)  \
  P.VU.vstart->write(0);
...
#define VI_VV_LOOP_NARROW(BODY) \
  require(P.core_type == STANDARD || (P.VU.vflmul < 1.0) || (P.VU.vl->read() <= P.VU.vlmax/2)); \
  VI_CHECK_SDS(true); \
  VI_LOOP_BASE \
  if (sew == e8) { \
...
#define VI_LD(stride, offset, elt_width, is_mask_ldst, stride_val) \
  const reg_t nf = insn.v_nf() + 1; \
  const reg_t vl = is_mask_ldst ? ((P.VU.vl->read() + 7) / 8) : P.VU.vl->read(); \
  const reg_t baseAddr = RS1; \
  const reg_t vd = insn.rd(); \
  int tval_type; \
  if (P.core_type == STANDARD) tval_type = 0; \
  else if (P.is_vpu()) tval_type = 6; \
  else tval_type = 4; \
  if (((int)(stride_val) > 0) && ((__builtin_popcount(stride_val) != 1) || ((int)(stride_val) >= P.VU.vlenb))) tval_type = 0; \
  else if (((int)(stride_val) < 0) && ((__builtin_popcount(-(stride_val)) != 1) || ((int)(-stride_val) >= P.VU.vlenb))) tval_type = 0; \
  else if (stride_val == 0) tval_type = 0; \
  VI_CHECK_LOAD(elt_width, is_mask_ldst); \
  require(!((P.is_vpu() || P.core_type == SARGANTANA) && nf != 1)); \
  require(!(P.core_type != STANDARD && emul > 1)); \
  for (reg_t i = 0; i < vl; ++i) { \
    VI_ELEMENT_SKIP(i); \
    VI_STRIP(i); \
    if (P.core_type == STANDARD) P.VU.vstart->write(i); \
    for (reg_t fn = 0; fn < nf; ++fn) { \
      elt_width##_t val = MMU.load<elt_width##_t>( \
        baseAddr + (stride) + (offset) * sizeof(elt_width##_t), 0, tval_type); \
      if (P.core_type == STANDARD) P.VU.elt<elt_width##_t>(vd + fn * emul, vreg_inx, true) = val; \
      P.VU.mem_addr_array[i*nf+fn] = (baseAddr + (stride) + (offset) * sizeof(elt_width##_t)); \
      switch (P.VU.vsew) { \
        case e8: P.VU.mem_data_array[i*nf+fn] = (uint8_t) (val); break; \
        case e16: P.VU.mem_data_array[i*nf+fn] = (uint16_t) (val); break; \
        case e32: P.VU.mem_data_array[i*nf+fn] = (uint32_t) (val); break; \
        default: P.VU.mem_data_array[i*nf+fn] = (uint64_t) (val); break; \
      } \
    } \
  } \
  if (P.core_type != STANDARD) { \
    for (reg_t i = 0; i < vl; ++i) { \
        VI_ELEMENT_SKIP(i); \
        VI_STRIP(i); \
        for (reg_t fn = 0; fn < nf; ++fn) { \
            P.VU.elt<elt_width##_t>(vd + fn * emul, vreg_inx, true) = P.VU.mem_data_array[i*nf+fn]; \
        } \
     }\
  } \
  VI_TAIL_AGNOSTIC_LD(elt_width, is_mask_ldst) \
  P.VU.vstart->write(0);
...
#define VI_LD_INDEX(elt_width, is_seg) \
  ...
  for (reg_t i = 0; i < vl; ++i) { \
    ...
    for (reg_t fn = 0; fn < nf; ++fn) { \
      switch (P.VU.vsew) { \
        case e8: \
          if (P.core_type == STANDARD) P.VU.elt<uint8_t>(vd + fn * flmul, vreg_inx, true) = \
            MMU.load<uint8_t>(baseAddr + index[i] + fn * 1, 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 1); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint8_t>(baseAddr + index[i] + fn * 1, 0, tval_type); \
          break; \
        case e16: \
          if (P.core_type == STANDARD) P.VU.elt<uint16_t>(vd + fn * flmul, vreg_inx, true) = \
            MMU.load<uint16_t>(baseAddr + index[i] + fn * 2, 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 2); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint16_t>(baseAddr + index[i] + fn * 2, 0, tval_type); \
          break; \
        case e32: \
          if (P.core_type == STANDARD) P.VU.elt<uint32_t>(vd + fn * flmul, vreg_inx, true) = \
            MMU.load<uint32_t>(baseAddr + index[i] + fn * 4, 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 4); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint32_t>(baseAddr + index[i] + fn * 4, 0, tval_type); \
          break; \
        default: \
          if (P.core_type == STANDARD) P.VU.elt<uint64_t>(vd + fn * flmul, vreg_inx, true) = \
            MMU.load<uint64_t>(baseAddr + index[i] + fn * 8, 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 8); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint64_t>(baseAddr + index[i] + fn * 8, 0, tval_type); \
          break; \
      } \
    } \
  } \
  for (reg_t i = 0; P.core_type != STANDARD && i < vl; ++i) { \
    VI_ELEMENT_SKIP(i); \
    VI_STRIP(i); \
    for (reg_t fn = 0; fn < nf; ++fn) { \
      switch (P.VU.vsew) { \
        case e8: \
          P.VU.elt<uint8_t>(vd + fn * flmul, vreg_inx, true) = P.VU.mem_data_array[i*nf+fn]; \
          break; \
        case e16: \
          P.VU.elt<uint16_t>(vd + fn * flmul, vreg_inx, true) = P.VU.mem_data_array[i*nf+fn]; \
          break; \
        case e32: \
          P.VU.elt<uint32_t>(vd + fn * flmul, vreg_inx, true) = P.VU.mem_data_array[i*nf+fn]; \
          break; \
        default: \
          P.VU.elt<uint64_t>(vd + fn * flmul, vreg_inx, true) = P.VU.mem_data_array[i*nf+fn]; \
          break; \
      } \
    } \
  } \
...
#define VI_ST(stride, offset, elt_width, is_mask_ldst, is_strided) \
  const reg_t nf = insn.v_nf() + 1; \
  const reg_t vl = is_mask_ldst ? ((P.VU.vl->read() + 7) / 8) : P.VU.vl->read(); \
  const reg_t baseAddr = RS1; \
  const reg_t vs3 = insn.rd(); \
  int tval_type, curr_tval_type; \
  int cache_line_addr_mask, cache_line_log_bytes; \
  if (P.core_type == STANDARD) tval_type = 0; \
  else if (insn.v_vm() == 0 || is_strided) tval_type = 0; \
  else if (P.is_vpu()) { \
    tval_type = 6; \
    cache_line_addr_mask = 0x3f; \
  } \
  else { \
    tval_type = 4; \
    cache_line_addr_mask = 0xf; \
  } \
  VI_CHECK_STORE(elt_width, is_mask_ldst); \
  require(!((P.is_vpu() || P.core_type==SARGANTANA) && nf != 1)); \
  require(!(P.core_type != STANDARD && emul > 1)); \
  int curr_req_tval_type = 0; \
  int curr_req_remaining_bytes = 0; \
  for (reg_t i = 0; i < vl; ++i) { \
    VI_STRIP(i) \
    VI_ELEMENT_SKIP(i); \
    if (P.core_type == STANDARD) P.VU.vstart->write(i); \
    for (reg_t fn = 0; fn < nf; ++fn) { \
      ...
      curr_tval_type = tval_type; \
      if ((tval_type != 0) && (curr_req_remaining_bytes == 0)) { \
        reg_t aux_addr = (baseAddr + (stride) + (offset) * sizeof(elt_width##_t)); \
        int remaining_bytes = (fn + nf * emul * (vl - i)) * sizeof(elt_width##_t); \
        int max_req_bytes, cache_line_size_b; \
        cache_line_size_b = 1 << tval_type; \
        /* Check remaining bytes */ \
        if (remaining_bytes >= cache_line_size_b) max_req_bytes = cache_line_size_b; \
        else { \
          int req_size = tval_type - 1; \
          int req_size_b; \
          for (req_size_b = 1 << req_size; (remaining_bytes < req_size_b) && (req_size > 0); req_size--, req_size_b = 1 << req_size); \
          max_req_bytes = req_size_b; \
          curr_tval_type = req_size; \
        } \
        /* Check alignment */ \
        int alignment = max_req_bytes - 1;  /* 0x3f for 6, 0xf for 4 */ \
        for (alignment = max_req_bytes - 1; ((aux_addr & alignment) != 0) && (alignment != 0); alignment = alignment >> 1) { \
           curr_tval_type--; \
        } \
        curr_req_tval_type = curr_tval_type; \
        curr_req_remaining_bytes = 1 << curr_tval_type; \
      } \
      if (tval_type != 0) curr_req_remaining_bytes -= sizeof(elt_width##_t); \
      MMU.store<elt_width##_t>( \
        baseAddr + (stride) + (offset) * sizeof(elt_width##_t), val, 0, curr_req_tval_type); \
      P.VU.mem_addr_array[i*nf+fn] = (baseAddr + (stride) + (offset) * sizeof(elt_width##_t)); \
      P.VU.mem_data_array[i*nf+fn] = (val); \
    } \
  } \
  P.VU.vstart->write(0);
    ...
    for (reg_t fn = 0; fn < nf; ++fn) { \
      ...
      case e8: \
        MMU.store<uint8_t>(baseAddr + index[i] + fn * 1, \
          P.VU.elt<uint8_t>(vs3 + fn * flmul, vreg_inx), 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 1); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint8_t>(baseAddr + index[i] + fn * 1, 0, tval_type); \
        break; \
      case e16: \
        MMU.store<uint16_t>(baseAddr + index[i] + fn * 2, \
          P.VU.elt<uint16_t>(vs3 + fn * flmul, vreg_inx), 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 2); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint16_t>(baseAddr + index[i] + fn * 2, 0, tval_type); \
        break; \
      case e32: \
        MMU.store<uint32_t>(baseAddr + index[i] + fn * 4, \
          P.VU.elt<uint32_t>(vs3 + fn * flmul, vreg_inx), 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 4); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint32_t>(baseAddr + index[i] + fn * 4, 0, tval_type); \
        break; \
      default: \
        MMU.store<uint64_t>(baseAddr + index[i] + fn * 8, \
          P.VU.elt<uint64_t>(vs3 + fn * flmul, vreg_inx), 0, tval_type); \
          P.VU.mem_addr_array[i*nf+fn] = (baseAddr + index[i] + fn * 8); \
          P.VU.mem_data_array[i*nf+fn] = MMU.load<uint64_t>(baseAddr + index[i] + fn * 8, 0, tval_type); \
        break; \
      } \
...
#define VI_LDST_FF(elt_width) \
    ...
    for (reg_t fn = 0; fn < nf; ++fn) { \
      ...
      try { \
        val = MMU.load<elt_width##_t>( \
          baseAddr + (i * nf + fn) * sizeof(elt_width##_t), 0, tval_type); \
      } catch (trap_t& t) { \
        if (i == 0) \
      ...
      P.VU.mem_addr_array[i*nf+fn] = (baseAddr + (i * nf + fn) * sizeof(elt_width##_t)); \
      P.VU.mem_data_array[i*nf+fn] = (val); \
    } \
    \
    if (early_stop) { \
  ...
  VI_TAIL_AGNOSTIC_LD(elt_width, 0) \
  p->VU.vstart->write(0);
...
#define VI_LD_WHOLE(elt_width) \
  ...
  require(!((P.is_vpu() || P.core_type == SARGANTANA) && len != 1)); \
    ...
    if (off) { \
      for (reg_t pos = off; pos < elt_per_reg; ++pos) { \
        auto val = MMU.load<elt_width##_t>(baseAddr + \
          (i * elt_per_reg + pos) * sizeof(elt_width ## _t), 0, tval_type); \
        if (P.core_type == STANDARD) P.VU.elt<elt_width ## _t>(vd + i, pos, true) = val; \
        P.VU.mem_addr_array[i * elt_per_reg + pos] = (baseAddr + (i * elt_per_reg + pos) * sizeof(elt_width##_t)); \
        P.VU.mem_data_array[i * elt_per_reg + pos] = (val); \
        if (P.core_type == STANDARD) P.VU.vstart->write(P.VU.vstart->read() + 1); \
      } \
      ++i; \
    } \
    for (; i < len; ++i) { \
  ...
  if (P.core_type != STANDARD && P.VU.vstart->read() < size) { \
    reg_t i = P.VU.vstart->read() / elt_per_reg; \
    reg_t off = P.VU.vstart->read() % elt_per_reg; \
    if (off) { \
      for (reg_t pos = off; pos < elt_per_reg; ++pos) { \
        P.VU.elt<elt_width ## _t>(vd + i, pos, true) = P.VU.mem_data_array[i * elt_per_reg + pos]; \
      } \
      ++i; \
    } \
    for (; i < len; ++i) { \
  ...
  if (P.VU.vstart->read() < size) { \
    ...
    if (off) { \
      for (reg_t pos = off; pos < P.VU.vlenb; ++pos) { \
        ...
        MMU.store<uint8_t>(baseAddr + (i * P.VU.vlenb + pos), val, 0, tval_type); \
        P.VU.mem_addr_array[i * P.VU.vlenb + pos] = baseAddr + i * P.VU.vlenb + pos; \
        P.VU.mem_data_array[i * P.VU.vlenb + pos] = val; \
        if (P.core_type == STANDARD) P.VU.vstart->write(P.VU.vstart->read() + 1); \
      } \
      i++; \
    } \
...
#define VI_AMO(op, type, idx_type) \
  ...
  for (reg_t i = P.VU.vstart->read(); i < vl; ++i) { \
    VI_ELEMENT_SKIP(i); \
    VI_STRIP(i); \
    if (P.core_type != SARGANTANA) P.VU.vstart->write(i); \
    switch (P.VU.vsew) { \
    case e32: { \
...
#define VI_VFP_LOOP_END(is_widening) \
  } \
  VI_TAIL_AGNOSTIC_BASIC(is_widening) \
  P.VU.vstart->write(0); \
...
#define VI_VFP_CVT_LOOP(CVT_PARAMS, CHECK, BODY, is_widening) \
  CHECK \
  VI_VFP_LOOP_SCALE_BASE \
  CVT_PARAMS \
  BODY \
  set_fp_exceptions; \
  VI_VFP_LOOP_END(is_widening)
...
#define VI_VFP_WCVT_FP_TO_FP(BODY8, BODY16, BODY32, \
  ...
  switch (P.VU.vsew) { \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_FP_PARAMS(16, 32), CHECK16, BODY16, 1); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_FP_PARAMS(32, 64), CHECK32, BODY32, 1); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_WCVT_INT_TO_FP(BODY8, BODY16, BODY32, \
  ...
  switch (P.VU.vsew) { \
    case e8: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(8, 16, sign), CHECK8, BODY8, 1); } \
      break; \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(16, 32, sign), CHECK16, BODY16, 1); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(32, 64, sign), CHECK32, BODY32, 1); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_WCVT_FP_TO_INT(BODY8, BODY16, BODY32, \
  ...
  switch (P.VU.vsew) { \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(16, 32, sign), CHECK16, BODY16, 1); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(32, 64, sign), CHECK32, BODY32, 1); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_NCVT_FP_TO_FP(BODY8, BODY16, BODY32, \
  ...
  switch (P.VU.vsew) { \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_FP_PARAMS(32, 16), CHECK16, BODY16, 0); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_FP_PARAMS(64, 32), CHECK32, BODY32, 0); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_NCVT_INT_TO_FP(BODY8, BODY16, BODY32, \
  ...
  switch (P.VU.vsew) { \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(32, 16, sign), CHECK16, BODY16, 0); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_INT_TO_FP_PARAMS(64, 32, sign), CHECK32, BODY32, 0); } \
      break; \
    default: \
      require(0); \
      break; \
...
#define VI_VFP_NCVT_FP_TO_INT(BODY8, BODY16, BODY32, \
  ...
  switch (P.VU.vsew) { \
    case e8: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(16, 8, sign), CHECK8, BODY8, 0); } \
      break; \
    case e16: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(32, 16, sign), CHECK16, BODY16, 0); } \
      break; \
    case e32: \
      { VI_VFP_CVT_LOOP(CVT_FP_TO_INT_PARAMS(64, 32, sign), CHECK32, BODY32, 0); } \
      break; \
    default: \
      require(0); \
      break; \
