...
plic_t::plic_t(std::vector<processor_t*>& procs, bool smode, uint32_t ndev)
  : procs(procs), contexts( PLIC_MAX_CONTEXTS * (smode ? 2 : 1))
{
  size_t contexts_per_hart = smode ? 2 : 1;

  for(size_t i = 0; i < procs.size(); i++){
    contexts_per_hart = contexts_per_hart * PLIC_MAX_CONTEXTS;
    //fprintf(stdout, "PROCS SIZE %d ID %d contexts_per_hart %d \n", procs.size(), procs[i]->get_id(), contexts_per_hart);
  }
  ...
  //fprintf(stdout, "PLIC.CC - PROCS SIZE %d CONTEXT SIZE %d\n", procs.size(), contexts.size() );
}
...
void plic_t::context_update(const plic_context_t *c)
{
  ...
  //fprintf(stdout, "PLIC.CC Context update - MIP value %lx\n", c->proc->state.mip->read() );
  c->proc->state.mip->backdoor_write_with_mask(mask, best_id ? mask : 0);
  //fprintf(stdout, "PLIC.CC Context update - MIP value after %lx\n", c->proc->state.mip->read() );
}
                                  ...
                                  reg_t offset, uint32_t val)
{
  ...
  //fprintf(stdout, "PLIC.CC ENABLE WRITE. num_ids_word %d id_word %d \n", num_ids_word, id_word);
  ...
  //fprintf(stdout, "PLIC.CC ENABLE WRITE. num_ids_word %d id_word %d old_val %x, new_val %x, val %x, enable[%d]=%x \n", num_ids_word, id_word, old_val, new_val, val, id_word, c->enable[id_word]);
  ...
  //context_update(c); //[TODO]done in set_external_interrupt -> make configurable
  return true;
}


void plic_t::get_plic_enable( int context, uint32_t* enable ){
  plic_context_t* c = &contexts[context];
  for( int i = 0; i < 8; i++){
    enable[i] = c->enable[i];
  }
}
