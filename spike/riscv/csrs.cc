...
tvec_csr_t::tvec_csr_t(processor_t* const proc, const reg_t addr, const reg_t val):
  csr_t(proc, addr),
  val(val) {
}
...
bool tvec_csr_t::unlogged_write(const reg_t val) noexcept {
  if ((val & 1) && ((proc->core_type == SARGANTANA) || (proc->core_type == LAGARTO_KA))) { // vectored
    this->val = val & ~(reg_t)(254); // Mask here is 111..1100000001. SARG and LKA require addr alignment to 64B if mode is vectored. LSB stays to encode the mode
  }
  else this->val = val & ~(reg_t)2;
  return true;
}
...
reg_t mstatus_csr_t::compute_mstatus_initial_value() const noexcept {
  ...
  return 0
         ...
         | (proc->is_vpu() || (proc->core_type == SARGANTANA) ? MSTATUS_VS : 0)
...
misa_csr_t::misa_csr_t(processor_t* const proc, const reg_t addr, const reg_t max_isa):
  ...
  max_isa(max_isa),
  // BSC implementations of misa are currently not writable
  write_mask((proc->core_type != STANDARD) ? 0 : max_isa & (0  // allow MAFDQCHV bits in MISA to be modified
                        | (1L << ('M' - 'A'))
                        | (1L << ('A' - 'A'))
                        | (1L << ('F' - 'A'))
                        | (1L << ('D' - 'A'))
